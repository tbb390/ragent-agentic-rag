...
export function KnowledgeDocumentsPage() {
  ...
  const loadChunkLogs = async (docId: string, background = false) => {
    if (!background) setLogLoading(true);
    try {
      const data = await getChunkLogsPage(docId, 1, 5);
      setLogData(data);
    } catch (error) {
  ...
  const activeLogStatus = logData?.records[0]?.status;
  const logTargetId = logTarget ? String(logTarget.id) : null;
  useEffect(() => {
    if (!logTargetId || (activeLogStatus !== "pending" && activeLogStatus !== "running")) return;
    const timer = window.setInterval(() => {
      void getChunkLogsPage(logTargetId, 1, 5).then(setLogData).catch(console.error);
    }, 2000);
    return () => window.clearInterval(timer);
  }, [logTargetId, activeLogStatus]);
      ...
      <Dialog open={Boolean(logTarget)} onOpenChange={(open) => (!open ? setLogTarget(null) : null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sidebar-scroll sm:max-w-[800px]" onOpenAutoFocus={(e) => e.preventDefault()} onCloseAutoFocus={(e) => { e.preventDefault(); requestAnimationFrame(() => (document.activeElement as HTMLElement)?.blur()); }}>
          ...
          {logLoading ? (
            ...
            <div className="space-y-4">
              {activeLogStatus === "pending" ? (
                <div className="flex items-center gap-3 border-l-2 border-amber-400 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                  <RefreshCw className="h-4 w-4 animate-spin" />
                  任务已写入本地事务，正在等待 RocketMQ 消费；页面每 2 秒自动刷新。
                </div>
              ) : null}
              {logData.records.map((log, index) => {
                const isPipelineLog = log.processMode?.toLowerCase() === "pipeline";
                const chunkLabel = isPipelineLog ? "数据通道耗时" : "分块耗时";
                return (
                <div key={log.id} className={cn("space-y-4", index > 0 && "border-t pt-4")}>
                  {/* 状态 + 基本信息 */}
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-500">
                    <span>{log.status === "pending" ? "排队时间" : "执行时间"}</span>
                    <span className="tabular-nums text-slate-700">{formatFullDateTime(log.startTime || log.createTime)}</span>
                    <span>~</span>
                    <span className="tabular-nums text-slate-700">{log.endTime ? formatFullDateTime(log.endTime) : log.status === "pending" ? "等待消费" : "进行中"}</span>
                  </div>
