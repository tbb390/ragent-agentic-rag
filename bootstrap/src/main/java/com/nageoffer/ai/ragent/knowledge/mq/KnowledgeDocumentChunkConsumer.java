...
public class KnowledgeDocumentChunkConsumer implements RocketMQListener<MessageWrapper<KnowledgeDocumentChunkEvent>> {
    ...
    public void onMessage(MessageWrapper<KnowledgeDocumentChunkEvent> message) {
        ...
        KnowledgeDocumentChunkEvent event = body instanceof KnowledgeDocumentChunkEvent chunkEvent
                ? chunkEvent
                : objectMapper.convertValue(body, KnowledgeDocumentChunkEvent.class);

        log.info("[消费者] 开始消费文档分块任务，docId={}, keys={}", event.getDocId(), message.getKeys());

        log.info("[consumer] knowledge document chunk payload type: {}", body == null ? null : body.getClass().getName());

        UserContext.set(LoginUser.builder().username(event.getOperator()).build());
        try {
            documentService.executeChunk(event.getDocId(), event.getLogId());
        } finally {
            UserContext.clear();
        }
    }
