...
public class KnowledgeDocumentServiceImpl implements KnowledgeDocumentService {
    ...
    public void startChunk(String docId) {
        ...
        messageQueueProducer.sendInTransaction(
                ...
                arg -> {
                    ...
                    log.info("[chunk] task queued, docId={}, logId={}, processMode={}",
                            docId, logId, processMode.getValue());
                }
        );
    }

    @Override
    public void executeChunk(String docId, String logId) {
        log.info("[chunk] execute chunk requested, docId={}, logId={}", docId, logId);
        KnowledgeDocumentDO documentDO = documentMapper.selectById(docId);
        if (documentDO == null) {
        ...
        runChunkTask(documentDO, logId);
    }

    private void runChunkTask(KnowledgeDocumentDO documentDO, String logId) {
        String docId = documentDO.getId();
        ProcessMode processMode = ProcessMode.normalize(documentDO.getProcessMode());
        log.info("[chunk] start chunk task, docId={}, processMode={}, fileUrl={}, fileType={}, fileSize={}",
                docId, processMode.getValue(), documentDO.getFileUrl(), documentDO.getFileType(), documentDO.getFileSize());

        KnowledgeDocumentChunkLogDO chunkLog = StrUtil.isNotBlank(logId) ? chunkLogMapper.selectById(logId) : null;
        if (chunkLog != null && DocumentStatus.SUCCESS.getCode().equals(chunkLog.getStatus())) {
            log.info("[chunk] duplicate delivery skipped, docId={}, logId={}", docId, logId);
            return;
        }
        if (chunkLog == null) {
            ...
            chunkLog.setStatus(DocumentStatus.RUNNING.getCode());
            chunkLog.setStartTime(new Date());
            chunkLogMapper.updateById(chunkLog);
        }
        log.info("[chunk] chunk log created, docId={}, logId={}", docId, chunkLog.getId());
        ...
        try {
            ...
            log.info("[chunk] persist chunks and vectors, docId={}, collectionName={}, chunkCount={}",
                    docId, collectionName, chunkResults == null ? 0 : chunkResults.size());
            ...
            log.info("[chunk] chunk task succeeded, docId={}, savedCount={}, totalDurationMs={}",
                    docId, savedCount, totalDuration);
        } catch (Exception e) {
    ...
    private ChunkProcessResult runChunkProcess(KnowledgeDocumentDO documentDO) {
        ...
        log.info("[chunk] open document stream, docId={}, fileUrl={}", documentDO.getId(), documentDO.getFileUrl());
        try (InputStream is = fileStorageService.openStream(documentDO.getFileUrl())) {
            log.info("[chunk] extract text with Tika, docId={}, docName={}", documentDO.getId(), documentDO.getDocName());
            ...
            log.info("[chunk] text extracted, docId={}, chars={}, durationMs={}",
                    documentDO.getId(), text == null ? 0 : text.length(), extractDuration);

            ChunkingStrategy chunkingStrategy = chunkingStrategyFactory.requireStrategy(chunkingMode);
            long chunkStart = System.currentTimeMillis();
            log.info("[chunk] split text, docId={}, strategy={}", documentDO.getId(), chunkingMode.getValue());
            List<VectorChunk> chunks = chunkingStrategy.chunk(text, config);
            long chunkDuration = System.currentTimeMillis() - chunkStart;
            log.info("[chunk] text split completed, docId={}, chunkCount={}, durationMs={}",
                    documentDO.getId(), chunks == null ? 0 : chunks.size(), chunkDuration);

            long embedStart = System.currentTimeMillis();
            log.info("[chunk] embed chunks, docId={}, embeddingModel={}, chunkCount={}",
                    documentDO.getId(), embeddingModel, chunks == null ? 0 : chunks.size());
            chunkEmbeddingService.embed(chunks, embeddingModel);
            long embedDuration = System.currentTimeMillis() - embedStart;
            log.info("[chunk] embedding completed, docId={}, durationMs={}", documentDO.getId(), embedDuration);
